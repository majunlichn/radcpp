...
static std::shared_ptr<spdlog::logger> g_DefaultLogger;
...
{
    g_DefaultLogger = spdlog::stdout_color_mt("Default");
    spdlog::set_default_logger(g_DefaultLogger);

    if (fileSink)
    {
        g_DefaultLogger->sinks().push_back(fileSink);
    }

#if defined(RAD_COMPILER_MSVC) && defined(_DEBUG)
    spdlog::sink_ptr msvcSink = std::make_shared<spdlog::sinks::msvc_sink_mt>();
    g_DefaultLogger->sinks().push_back(msvcSink);
#endif
...
{
    auto& sinks = g_DefaultLogger->sinks();
