...
class MLCpuTensorIterator : public MLTensorIterator
{
public:
    MLCpuTensorIterator(MLTensor* tensor) :
        MLTensorIterator(tensor)
    {
    }

    ~MLCpuTensorIterator() = default;
    ...
    void ForEachParallel(const ElementOp& op, size_t granularityND)
    {
        ...
        size_t coreCount = GetNumberOfPhysicalCores();
        do {
