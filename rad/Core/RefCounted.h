...
template<typename T>
void AddRef(const RefCounted<T>* p) noexcept;
template<typename T>
void DecRef(const RefCounted<T>* p) noexcept;
...
private:
    ...
    friend void AddRef<T>(const RefCounted<T>* p) noexcept;
    friend void DecRef<T>(const RefCounted<T>* p) noexcept;

}; // class RefCounted

template<class T>
inline void AddRef(const RefCounted<T>* p) noexcept
{
    p->m_refCount.fetch_add(1, std::memory_order_relaxed);
}

template<class T>
inline void DecRef(const RefCounted<T>* p) noexcept
{
